...
%% A. 变压器
fprintf('%s\n', sep);
fprintf('【A】变压器故障概率 (IEEE C57.91 热点 + Arrhenius-Weibull)\n');
fprintf('%-4s%-5s%-5s%10s%8s%10s%10s%12s%12s\n', ...
    'L','fb','tb','flow','K','thetaH','FAA','lambda/yr','Pf');
xf = fp_res.transformers;
for i = 1:numel(xf)
    d = xf(i);
    fprintf('L%-3d%-5d%-5d%10.2f%8.3f%10.1f%10.3f%12.4f%12.3e\n', ...
        d.L, d.fbus, d.tbus, d.flow, d.K, d.thetaH, d.FAA, d.lambda_yr, d.Pf);
end
